...
// Function to retrieve a secret version
function getSecretVersion($projectId, $secretId, $versionId)
{
    try {
        $client = new SecretManagerServiceClient();
        $name = $client->secretVersionName($projectId, $secretId, $versionId);
        $version = $client->accessSecretVersion($name);
 
        return $version->getPayload()->getData();
    } catch (\Google\ApiCore\ApiException $e) {
        // Handle API errors
        return "Error fetching secret version: " . $e->getMessage();
    } finally {
        $client->close();
    }
}
...
$container->set('cloudsql', function ($container) {
    ...
    $dbConn = getSecretVersion('prod-401919', 'CLOUDSQL_CONNECTION_NAME', 'latest');
    //$dbUser = getenv('CLOUDSQL_USER');
    $dbUser = getSecretVersion('prod-401919', 'CLOUDSQL_USER', 'latest');
    // $dbPass = getenv('CLOUDSQL_PASSWORD');
    $dbPass = getSecretVersion('prod-401919', 'CLOUDSQL_PASSWORD', 'latest');
